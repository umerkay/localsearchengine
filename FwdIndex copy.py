...
class FwdIndex:
    ...
    def generateFromFile(self, lexiconObj, invInd, file):
        ...
        with open(file, 'r') as f:
            ...
            for doc in data:
                lexWords = numpy.unique([doc[i] for i in range(1,len(doc))])
                self.docTable.append(doc[0])

                for (word, count) in zip(lexWords, lexWords):
                    tuple = [len(self.docTable) - 1, 3]
                    if(word in invInd.wordIDs):
                        invInd.wordIDs[word].append(tuple)
                    else:
                        invInd.wordIDs[word] = [tuple]
